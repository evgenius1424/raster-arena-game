...
export const buildRoutes = (
    ...
    router.get('/rooms', limiter, async (_req, res) =>
        res.json({ rooms: await roomService.listRooms() }),
    )

    router.get('/rooms/:roomCode/events', auth, async (req, res) => {
        const roomCode = roomCodeSchema.parse(req.params.roomCode)
        if (req.auth?.roomCode !== roomCode)
            return res
                .status(403)
                .json({ error: { code: 'forbidden', message: 'Session not for this room' } })
        ...
        req.app.locals.sseSubscribe(roomCode, req.auth.playerId, res)
        res.write('event: room.snapshot\n')
        res.write(`data: ${JSON.stringify(await roomService.getSnapshot(roomCode))}\n\n`)
    ...
    router.get('/session/me', auth, async (req, res) => {
        const roomCode = req.query.roomCode
            ? roomCodeSchema.parse(req.query.roomCode)
            : req.auth!.roomCode
        if (req.auth?.roomCode !== roomCode)
            return res
                .status(401)
                .json({ error: { code: 'unauthorized', message: 'Session room mismatch' } })
        const { room, player } = await roomService.resolveMember(roomCode, req.auth.playerId)
        res.json({
            player,
            room: { roomCode: room.roomCode, roomName: room.roomName, status: room.status },
        })
    })
